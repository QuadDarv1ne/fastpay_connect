...
class TestRobokassaWebhook:
    @patch("app.payment_gateways.robokassa.verify_signature", return_value=True)
    async def test_payment_succeeded(self, mock_verify):
        payload = {"event": "payment.succeeded", "payment_id": "123"}
        result = await handle_robokassa_webhook(payload, "sig")
    ...
    @patch("app.payment_gateways.robokassa.verify_signature", return_value=False)
