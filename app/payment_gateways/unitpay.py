...
def generate_signature(params: Dict[str, Any]) -> str:
    ...
    if not UNITPAY_SECRET_KEY:
        logger.warning("UNITPAY_SECRET_KEY is not configured")
        return ""
...
def create_payment(
    ...
    :return: Ответ от UnitPay или ошибка.
    """
    if not UNITPAY_API_KEY:
        logger.error("UNITPAY_API_KEY is not configured")
    ...
    try:
        ...
        logger.error(f"UnitPay HTTP error: {e}")
        try:
        ...
        logger.error("UnitPay request timeout")
        return {"error": "Payment gateway timeout"}

    except requests.exceptions.RequestException as e:
        logger.error(f"UnitPay request failed: {e}")
...
def verify_signature(params: Dict[str, Any], provided_signature: str) -> bool:
    ...
    if not UNITPAY_SECRET_KEY:
        logger.warning("UNITPAY_SECRET_KEY is not configured, skipping signature verification")
        return True
