...
class GooglePayGateway(BasePaymentGateway):
    ...
    async def refund_payment(
        ...
        """Возврат платежа через Google Pay/процессинг API."""
        logger.info(
            f"Google Pay: refunding payment {payment_id}, amount={amount}, reason={reason}"
        )
        # В реальной реализации здесь вызывается API процессингового банка
        # для возврата средств. Google Pay сам не обрабатывает возвраты —
    ...
    async def cancel_payment(self, payment_id: str) -> Dict[str, Any]:
        """Отмена платежа через Google Pay/процессинг API."""
        logger.info(f"Google Pay: cancelling payment {payment_id}")
        # В реальной реализации здесь вызывается API процессингового банка
